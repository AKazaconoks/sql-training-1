...
const insertMovies = (movies: Movie[]) => {
  return (
    `insert into MOVIES (imdb_id, popularity, budget, budget_adjusted, 
      revenue, revenue_adjusted, original_title, homepage, tagline, overview, runtime, release_date) values` +
    ...
    '${escape(movie.tagline || "")}', '${escape(movie.overview)}', ${movie.runtime}, '${escape(movie.releaseDate)}')`).join(",")
  );
};
