...
const insertMovieGenres = (
  ...
  const values = genres.map((genre) => {
    const genreRow = genreRows.find((g) => g.genre === genre);
    if (!genreRow) {
      throw new Error(`Genre "${genre}" not found in genreRows`);
    }
...
const insertMovieActors = (
  ...
  const values = actors.map((actor) => {
    const actorRow = actorRows.find((a) => a.full_name === actor);
    if (!actorRow) {
      throw new Error(`Actor "${actor}" not found in actorRows`);
    }
...
const insertMovieDirectors = (
  ...
  const values = directors.map((director) => {
    const directorRow = directorRows.find((d) => d.full_name === director);
    if (!directorRow) {
      throw new Error(`Director "${director}" not found in directorRows`);
    }
...
const insertMovieKeywords = (
  ...
  const values = keywords.map((keyword) => {
    const keywordRow = keywordRows.find((k) => k.keyword === keyword);
    if (!keywordRow) {
      throw new Error(`Keyword "${keyword}" not found in keywordRows`);
    }
...
const insertMovieProductionCompanies = (
  ...
  const values = productionCompanies.map((company) => {
    const companyRow = productionCompanyRows.find((p) => p.company_name === company);
    if (!companyRow) {
      throw new Error(`Keyword "${company}" not found in productionCompaniesRows`);
    }
    return `(${movieId}, ${companyRow.id})`;
  }).join(", ");
